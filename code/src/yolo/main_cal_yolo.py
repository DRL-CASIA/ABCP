...
from src.yolo.general_controller_yolo import GeneralController
from src.yolo.general_child_cal_yolo import GeneralChild
...
DEFINE_string("data_path", "/home/xuer/daima/NAS/enas-pruning/data/yolo", "")
# DEFINE_string("data_path", "/home/xuer/shuju/smatereye", "")
DEFINE_string("output_dir", "./output_yolo/yolo", "")
DEFINE_string("data_format", "NHWC", "'NHWC' or 'NCHW'")
DEFINE_string("weight_path", "/home/xuer/daima/NAS/enas-pruning/output_yolo", "")
    ...
    with tf.Session() as sess:
        ...
        while True:
          ...
          for child_epoch in range(1, 1 + first_stage_epochs + second_stage_epochs):
            ...
            curr_time = time.time()
            log_string = ""
            log_string += "epoch={:<6d}".format(epochs)
          ...
          for t in range(len(num_blocks)):
            ...
            change_num = sort_change_num[-1] if sort_change_num[-1] != 5 or np.shape(sort_change_num)[0] == 1 else \
            sort_change_num[-2]
            id += 2 * num_blocks[t] + 1
            num_id += 2 * num_blocks[t] + 1
            for i in change_ids:
              arc[i] = change_num if arc[i] != 5 else arc[i]
          ...
          loss, entropy, lr, gn, test_loss, bl, _ = sess.run(run_ops, feed_dict={controller_model.test_loss: test_epoch_loss, controller_model.flops: flops[0] / 1000000000.0})  # 训练控制器?
