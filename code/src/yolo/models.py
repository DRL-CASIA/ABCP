...
class Model(object):
  def __init__(self,
    ...
    with tf.device("/cpu:0"):
      ...
      self.epoch_lrdec_every = []
      if self.epoch_lrdec_ratio is not None:
        for i in self.epoch_lrdec_ratio:
          self.epoch_lrdec_every.append(int(i * self.num_train_batches))
        self.lr_values = [self.lr_init, 0.1 * self.lr_init, 0.01 * self.lr_init]
  ...
  def eval_once(self, sess, eval_set, feed_dict=None, verbose=False):
    ...
    for batch_id in range(num_batches):
      ...
      loss = sess.run(acc_op, feed_dict=feed_dict)
      total_loss += loss


      if verbose:
        sys.stdout.write("\r{:<5d}/{:>5d}".format(total_loss, total_exp))
    if verbose:
      print("")
  ...
  def _build_train(self):
    ...
    self.train_op, self.lr, self.grad_norm, self.optimizer = get_train_ops(
      ...
      epoch_lrdec=self.epoch_lrdec,
      epoch_lrdec_every=self.epoch_lrdec_every,
  ...
  def _build_valid(self):
    if self.x_valid is not None:
      ...
      valid_log_probs = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits, labels=self.y_valid)
      self.valid_loss = tf.reduce_mean(valid_log_probs)
  ...
  def _model(self, input_data, reuse=None):
