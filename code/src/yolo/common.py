...
def tools_pruning_conv(ini_gamma, weights, layer_sparsity):
    ...
    if layer_sparsity == 1.0:
        mask_gamma = tf.where(l1_w <= thre, x=zero_gamma, y=one_gamma)
    else:
        mask_gamma = tf.where(l1_w < thre, x=zero_gamma, y=one_gamma)
...
def residual_block_prune(input_data, input_channel, filter_num1, filter_num2, H, flops, trainable, name, arc, trans_ratio):
    ...
    with tf.variable_scope(name):
        ...
        input_data, flops, trans_ratio = enas_layer_pruning_conv(input_data, filters_shape=(1, 1, input_channel, filter_num1), H=H,
                                   trainable=trainable, name='conv1', prune_id=arc[0], trans_ratio=trans_ratio, flops=flops)

        input_data, flops, trans_ratio = enas_layer_pruning_conv(input_data, filters_shape=(3, 3, filter_num1, filter_num2), H=H,
                                   trainable=trainable, name='conv2', prune_id=arc[1], trans_ratio=trans_ratio, flops=flops)
...
def enas_layer_pruning_conv(input_data, filters_shape, H, trainable, name, prune_id, trans_ratio, flops, downsample=False, activate=True, bn=True):
  ...
  ep_outs = {}
  with tf.variable_scope("ratio_0"):
    ep_outs[tf.equal(prune_id, 0)] = lambda: convolutional_prune(input_data, filters_shape, trainable, name, 0.0, downsample, activate, bn)
  with tf.variable_scope("ratio_20"):
    ep_outs[tf.equal(prune_id, 1)] = lambda: convolutional_prune(input_data, filters_shape, trainable, name, 0.2, downsample, activate, bn)
  with tf.variable_scope("ratio_40"):
    ep_outs[tf.equal(prune_id, 2)] = lambda: convolutional_prune(input_data, filters_shape, trainable, name, 0.4, downsample, activate, bn)
  with tf.variable_scope("ratio_60"):
    ep_outs[tf.equal(prune_id, 3)] = lambda: convolutional_prune(input_data, filters_shape, trainable, name, 0.6, downsample, activate, bn)
  with tf.variable_scope("ratio_80"):
    ep_outs[tf.equal(prune_id, 4)] = lambda: convolutional_prune(input_data, filters_shape, trainable, name, 0.8, downsample, activate, bn)
  with tf.variable_scope("ratio_100"):
    ep_outs[tf.equal(prune_id, 5)] = lambda: convolutional_prune(input_data, filters_shape, trainable, name, 1.0, downsample, activate, bn)

  ep_outs = tf.case(ep_outs, default=lambda: convolutional_prune(input_data, filters_shape, trainable, name, 0.0, downsample, activate, bn), exclusive=True)

  remain_ratio = utils.remain_ratio(prune_id)
...
def maintain_pruning_conv(input_data, filters_shape, H, trainable, name, prune_id, trans_ratio, flops,  downsample=False, activate=True, bn=True):

  C_in = tf.cond(tf.equal(trans_ratio, 1.0), lambda: tf.to_int32(tf.multiply(tf.to_float(filters_shape[2]), trans_ratio)), lambda: tf.add(tf.to_int32(tf.multiply(tf.to_float(filters_shape[2]), trans_ratio)), 1))
  ep_outs = {}
  with tf.variable_scope("ratio_0"):
    ep_outs[tf.equal(prune_id, 0)] = lambda: convolutional_maintain(input_data, filters_shape, trainable, name, 0.0, downsample, activate, bn)
  with tf.variable_scope("ratio_20"):
    ep_outs[tf.equal(prune_id, 1)] = lambda: convolutional_maintain(input_data, filters_shape, trainable, name, 0.2, downsample, activate, bn)
  with tf.variable_scope("ratio_40"):
    ep_outs[tf.equal(prune_id, 2)] = lambda: convolutional_maintain(input_data, filters_shape, trainable, name, 0.4, downsample, activate, bn)
  with tf.variable_scope("ratio_60"):
    ep_outs[tf.equal(prune_id, 3)] = lambda: convolutional_maintain(input_data, filters_shape, trainable, name, 0.6, downsample, activate, bn)
  with tf.variable_scope("ratio_80"):
    ep_outs[tf.equal(prune_id, 4)] = lambda: convolutional_maintain(input_data, filters_shape, trainable, name, 0.8, downsample, activate, bn)
  with tf.variable_scope("ratio_100"):
    ep_outs[tf.equal(prune_id, 5)] = lambda: convolutional_maintain(input_data, filters_shape, trainable, name, 1.0, downsample, activate, bn)

  ep_outs = tf.case(ep_outs, default=lambda: convolutional_maintain(input_data, filters_shape, trainable, name, 0.0, downsample, activate, bn), exclusive=True)
