...
class GeneralController(Controller):
  ...
  def _create_params(self):
    ...
    with tf.variable_scope(self.name, initializer=initializer):
      ...
      if self.search_whole_channels:
        ...
        with tf.variable_scope("softmax_res"):
  ...
  def _sample_prune_ratio(self, inputs, prev_c, prev_h):
    next_c, next_h = stack_lstm(inputs, prev_c, prev_h, self.w_lstm)
    prev_c, prev_h = next_c, next_h
    logit_mu = tf.matmul(next_h[-1],
                         self.w_soft_mu)
    logit_rou = tf.matmul(next_h[-1],
                          self.w_soft_rou)
    ...
    mu = tf.multiply(tf.to_float(2), logit_mu[0])
    sigma = tf.add(0.1, tf.sqrt(tf.exp(logit_rou[0])))
    normal_dist = tf.distributions.Normal(mu, sigma)
    prune_ratio = tf.clip_by_value(normal_dist.sample(1), 0, 0.9)

    log_prob = normal_dist.log_prob(prune_ratio)
    entropy = normal_dist.entropy()

    inputs = tf.squeeze(tf.nn.embedding_lookup(self.w_emb, tf.to_int32(tf.round(prune_ratio))), axis=1)

    return prune_ratio, prev_c, prev_h, inputs, log_prob, entropy

  def _inherite_tensors(self, prune_ratio, prev_c, prev_h, inputs, log_prob, entropy):
    return prune_ratio, prev_c, prev_h, inputs, log_prob, entropy
  ...
  def _sample_if_resprune(self, inputs, prev_c, prev_h):
    ...
    # change to prune
    prune_ratio = tf.cond(tf.equal(if_prune[0], 1), lambda: tf.Variable([1.0], dtype=tf.float32, trainable=False), lambda: tf.Variable([0.0], dtype=tf.float32, trainable=False))
    prune_ratio = tf.reshape(prune_ratio, [1, 1])
    ...
    prune_ratio, prev_c, prev_h, inputs, log_prob, entropy = tf.cond(tf.equal(if_prune[0], 1), lambda: self._inherite_tensors(prune_ratio, prev_c, prev_h, inputs, log_prob, entropy), lambda: self._sample_prune_ratio(inputs, prev_c, prev_h))
    return prune_ratio, prev_c, prev_h, inputs, log_prob, entropy, res_log_prob, res_entropy, if_prune
  ...
  def _build_sampler(self):
    ...
    for layer_id in range(self.num_layers):
      if layer_id in self.res_id:
        print(1)
        prune_ratio, prev_c, prev_h, inputs, log_prob, entropy, res_log_prob, res_entropy, if_prune = self._sample_if_resprune(inputs, prev_c, prev_h)
        res_prune.append(if_prune)
        arc_seq.append(prune_ratio)
        ...
        print(2)
        prune_ratio, prev_c, prev_h, inputs, log_prob, entropy = tf.cond(tf.equal(if_prune[0], 1),
                                                                     lambda: self._inherite_tensors(prune_ratio, prev_c,
                                                                                                    prev_h, inputs,
                                                                                                    log_prob, entropy),
                                                                     lambda: self._sample_prune_ratio(inputs, prev_c, prev_h))
        arc_seq.append(prune_ratio)
        log_probs.append(log_prob)
        entropys.append(entropy)
        ...
        print(3)
        prune_ratio, prev_c, prev_h, inputs, log_prob, entropy = self._sample_prune_ratio(inputs, prev_c, prev_h)
        arc_seq.append(prune_ratio)
