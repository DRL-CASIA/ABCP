...
import common_continuous_mv as common
import tensorflow as tf
import utils


def darknet53_prune(input_data, trainable, arc, trans_ratio, flops):

    with tf.variable_scope('darknet'):

        input_data, flops, trans_ratio = common.enas_layer_pruning_conv(input_data, filters_shape=(3, 3,  3,  32), H=416, trainable=trainable, name='conv0', prune_ratio=arc[0], trans_ratio=trans_ratio, flops=flops)
        input_data, flops, trans_ratio = common.enas_layer_pruning_conv(input_data, filters_shape=(3, 3, 32,  64), H=208,
                                          trainable=trainable, name='conv1', downsample=True, prune_ratio=arc[1], trans_ratio=trans_ratio, flops=flops)
        ...
        input_data, flops, trans_ratio = common.enas_layer_pruning_conv(input_data, filters_shape=(3, 3,  64, 128), H=104,
                                          trainable=trainable, name='conv4', downsample=True, prune_ratio=arc[4], trans_ratio=trans_ratio, flops=flops)

        input_data, flops, trans_ratio = common.residual_block_prune(input_data, 128,  64, 128,  H=104,trainable=trainable, name='residual1', arc=arc[5:7], trans_ratio=trans_ratio, flops=flops)
        ...
        input_data, flops, trans_ratio = common.enas_layer_pruning_conv(input_data, filters_shape=(3, 3, 128, 256), H=52,
                                          trainable=trainable, name='conv9', downsample=True, prune_ratio=arc[9], trans_ratio=trans_ratio, flops=flops)
        ...
        input_data, flops, trans_ratio = common.enas_layer_pruning_conv(input_data, filters_shape=(3, 3, 256, 512), H=26,
                                          trainable=trainable, name='conv26', downsample=True, prune_ratio=arc[26], trans_ratio=trans_ratio, flops=flops)
        ...
        input_data, flops, trans_ratio = common.enas_layer_pruning_conv(input_data, filters_shape=(3, 3, 512, 1024), H=13,
                                          trainable=trainable, name='conv43', downsample=True, prune_ratio=arc[43], trans_ratio=trans_ratio, flops=flops)
...
def darknet53_maintain(input_data, trainable, arc, trans_ratio, flops):

    with tf.variable_scope('darknet'):


        input_data, flops, trans_ratio = common.maintain_pruning_conv(input_data, filters_shape=(3, 3,  3,  32), H=416, trainable=trainable, name='conv0', prune_ratio=arc[0], trans_ratio=trans_ratio, flops=flops)
        ...
        route_1 = tf.identity(input_data)
        input_data, flops, trans_ratio = common.maintain_pruning_conv(route_1, filters_shape=(3, 3, 256, 512), H=26,
                                          trainable=trainable, name='conv26', downsample=True, prune_ratio=arc[26], trans_ratio=trans_ratio, flops=flops)
        ...
        route_2 = tf.identity(input_data)
        input_data, flops, trans_ratio = common.maintain_pruning_conv(route_2, filters_shape=(3, 3, 512, 1024), H=13,
                                          trainable=trainable, name='conv43', downsample=True, prune_ratio=arc[43], trans_ratio=trans_ratio, flops=flops)
