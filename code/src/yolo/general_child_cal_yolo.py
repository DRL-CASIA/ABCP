...
import src.yolo.common as common
import src.yolo.backbone as backbone
...
class GeneralChild(object):
  ...
  def _model(self, input_data, trainable, reuse=tf.AUTO_REUSE):
    with tf.variable_scope(self.name, reuse=reuse):
        flops = 0
        flops = tf.Variable(0, trainable=False)
        trans_ratio = tf.Variable(1.0, trainable=False)
        ...
        conv_lobj_branch, flops, _ = common.enas_layer_pruning_conv(input_data, (3, 3, 512, 1024), 13, trainable, 'conv_lobj_branch', self.sample_arc[57], trans_ratio=trans_ratio, flops=flops)
        # lbbox
        ...
        conv_mobj_branch, flops, _ = common.enas_layer_pruning_conv(input_data, (3, 3, 256, 512), 26, trainable, 'conv_mobj_branch', self.sample_arc[64], trans_ratio=trans_ratio, flops=flops)
        # mbbox
        ...
        conv_sobj_branch, flops, _ = common.enas_layer_pruning_conv(input_data, (3, 3, 128, 256), 52, trainable, 'conv_sobj_branch', self.sample_arc[71], trans_ratio=trans_ratio, flops=flops)
        # sbbox
  ...
  def _model_train(self, input_data, trainable, reuse=tf.AUTO_REUSE):
    with tf.variable_scope(self.name, reuse=reuse):
        ...
        conv_lobj_branch, flops, _ = common.maintain_pruning_conv(input_data, (3, 3, 512, 1024), 13, trainable, 'conv_lobj_branch', self.sample_arc[57], trans_ratio=trans_ratio, flops=flops)
        # lbbox
        ...
        conv_mobj_branch, flops, _ = common.maintain_pruning_conv(input_data, (3, 3, 256, 512), 26, trainable, 'conv_mobj_branch', self.sample_arc[64], trans_ratio=trans_ratio, flops=flops)
        # mbbox
        ...
        conv_sobj_branch, flops, _ = common.maintain_pruning_conv(input_data, (3, 3, 128, 256), 52, trainable, 'conv_sobj_branch', self.sample_arc[71], trans_ratio=trans_ratio, flops=flops)
        # sbbox
  ...
  def connect_controller(self, controller_model):
    if self.fixed_arc is None:
      self.sample_arc = tf.placeholder(dtype=np.int32, shape=(self.num_layers), name="sample_arc")
    else:
