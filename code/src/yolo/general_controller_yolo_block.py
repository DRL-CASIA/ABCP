...
class GeneralController(Controller):
  ...
  def _sample_if_resprune(self, inputs, prev_c, prev_h):
    ...
    # change to prune id '5'
    branch_id = tf.cond(tf.equal(if_prune[0], 1), lambda: tf.Variable([5], trainable=False), lambda: tf.Variable([0], trainable=False))
    return branch_id, prev_c, prev_h, inputs, res_log_prob, res_entropy
  ...
  def _build_sampler(self):
    ...
    for layer_id in range(self.num_layers):
      if layer_id in self.res_id:
        print(1)
        branch_id, prev_c, prev_h, inputs, res_log_prob, res_entropy = self._sample_if_resprune(inputs, prev_c, prev_h)
        ...
        print(2)
        branch_id, prev_c, prev_h, inputs, res_log_prob, res_entropy = self._inherite_tensors(branch_id, prev_c, prev_h, inputs, res_log_prob, res_entropy)        
        arc_seq.append(branch_id)

      else:
        print(3)
        branch_id = tf.Variable([0], trainable=False)
        arc_seq.append(branch_id)
    ...
    self.sample_entropy = tf.reduce_sum(res_entropys)

    res_log_probs = tf.stack(res_log_probs)
    self.sample_log_prob = tf.reduce_sum(res_log_probs)
